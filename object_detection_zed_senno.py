...
#Defines the range/area to refer the 3D depth data from the centre of the box(x_center,y_center).
    ...
    for i in range(num_detections):
        if scores_[i] > confidence:
            ...
            ymin, xmin, ymax, xmax = box #box gives top left and bottom right coordinates
                ...
                if new_data:
                    ...
                    for i, box in enumerate(boxes):
                        if scores[i] > confidence:
                            row_data = box.tolist()
                            row_data.insert(0, frame)
                            row_data.append(scores[i])
                            row_data.append(classes[i])

                            np.savetxt(f, (row_data,), delimiter=',')
                    ...
                    #self.export_CSV(self.ymin_n, self.xmin_n, self.ymax_n, self.xmax_n)
